...
import { buildFewShotBlock } from './lib/few-shot-examples.js';
import { QUALITY_SYSTEM_RULES, verifyGeneratedQuestions } from './lib/question-quality.js';
import {
  ENTERTAINMENT_KEYWORDS,
  filterEntertainment,
  isBeforeAndAfterCategory,
  isClueIncomplete,
  isListQuestionBanned,
  isQuestionBanned,
  shuffleArray,
} from '../shared/question-filters.js';
import { makeQuestionId } from '../shared/question-ids.js';
import { attachAlternates } from '../shared/alternates.js';
import {
  loadQuestionLedger,
  saveQuestionLedger,
  loadUsedQuestionIds,
  saveUsedQuestionIds,
} from './lib/ledger-io.js';
import {
  buildRoundAlternates,
  DIRECT_STRATEGY_ROUNDS,
  ARCHIVE_SELECTION_ROUND_COUNT,
} from './lib/generator/orchestrator.js';
...
// Shared system prompt for LLM-generated trivia rounds. The base carries the
// long-standing difficulty guidance; QUALITY_SYSTEM_RULES is appended unless
// disabled via DISABLE_QUALITY_RULES=1 or setQualityRulesEnabled(false). The
// eval harness toggles it to A/B the rules against the pre-rules baseline.
const TRIVIA_GENERATOR_BASE_PROMPT = `You are a trivia question generator for a competitive pub trivia night. Your goal is to create questions that spark table debate and reward genuine knowledge.
...
let _qualityRulesEnabled = process.env.DISABLE_QUALITY_RULES !== '1';

/** Toggle whether QUALITY_SYSTEM_RULES is appended to the system prompt.
 *  Used by the eval harness to A/B the rules against the pre-rules baseline. */
export function setQualityRulesEnabled(enabled) { _qualityRulesEnabled = !!enabled; }
/** @returns {boolean} whether the quality rules are currently appended. */
export function isQualityRulesEnabled() { return _qualityRulesEnabled; }

/** Build the shared LLM system prompt, appending QUALITY_SYSTEM_RULES unless disabled. */
function buildTriviaSystemPrompt() {
  const rules = _qualityRulesEnabled ? `\n\n${QUALITY_SYSTEM_RULES}` : '';
  return `${TRIVIA_GENERATOR_BASE_PROMPT}${rules}\n\nAlways return valid JSON as specified in the prompt.`;
}
...
// ROUND_TEMPLATES: canonical round config from shared/round-definitions.js
...
function loadData() {
  ...
  const usedQuestions = loadUsedQuestionIds();
...
/**
 * Load banned questions from tracking files (main + UI export + ledger).
 */
 ...
 * Falls back to reuse (banned-only filter) when the pool is exhausted.
 */
function selectListRoundQuestion(usedQuestions = new Set(), bannedQuestions = []) {
  const allQuestions = loadListRoundQuestions();
  const isAvailable = (q, ignoreUsed = false) => {
    const listId = `list:${q.clue}`;
    if (!ignoreUsed && usedQuestions.has(listId)) return false;
    if (isListQuestionBanned(q.clue, bannedQuestions)) return false;
    return true;
  };

  let available = allQuestions.filter((q) => isAvailable(q));
  if (available.length === 0) {
    available = allQuestions.filter((q) => isAvailable(q, true));
    if (available.length > 0) {
      console.warn('List-round pool exhausted — reusing a previously used question (banned still excluded).');
    }
  }
  if (available.length === 0) {
    return null;
...
/**
 * Select up to count entertainment questions, excluding used and banned.
 */
...
- EXACTLY one option is correct; no alternate defensible answer. The other 3 must be verifiably WRONG yet plausible enough to fool someone who half-remembers the fact
- The clue must not reveal or hint at the answer
...
`
  ...
  prompt += buildFewShotBlock(5, { subType });
  ...
  try {
    const response = await openaiClient.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: buildTriviaSystemPrompt()
        },
        {
          role: 'user',
          content: prompt
...
async function generateOverUnderRound(usedQuestions = new Set()) {
  ...
  const avoidSection = usedClues.length > 0
    ...
    : '';
  const fewShotSection = buildFewShotBlock(2);
...
`
  ...
  // Universal guardrail for every subtype: one pinned answer, no giveaway clue.
  const universalRules = '\nEvery question must have exactly one correct answer with no alternate defensible answers, and the clue must never reveal or hint at that answer.\n';
  const prompt = prompts[subType] + universalRules + avoidSection + buildFewShotBlock(7, { subType });
...
async function generateLLMRound(roundNumber, subType = null, usedQuestions = new Set(), recentlyUsedSubTypes = []) {
  ...
  let result;
  if (roundNumber === 2) {
    result = await generateOverUnderRound(usedQuestions);
  } else if (roundNumber === 5) {
    result = await generateGameShowStyleRound(subType, usedQuestions);
  } else if (roundNumber === 7) {
    result = await generateMixingThingsUpRound(subType, usedQuestions);
  } else {
    throw new Error(`No LLM generator for round ${roundNumber}`);
  }

  // Optional post-generation verification (opt-in via VERIFY_LLM_QUESTIONS=1).
  // Off by default so the weekly run's cost/behavior is unchanged. Family Feud
  // is a curated DB question with intentional multiple answers — skip it.
  const verifyEnabled = process.env.VERIFY_LLM_QUESTIONS === '1';
  if (verifyEnabled && result?.questions?.length && result.subType !== 'family-feud') {
    console.log(`Verifying LLM round ${roundNumber} (${result.subType || 'n/a'}) — VERIFY_LLM_QUESTIONS=1`);
    const verified = await verifyGeneratedQuestions(result.questions, {
      openaiClient,
      roundLabel: template.title || `Round ${roundNumber}`,
      subType: result.subType
    });
    return { ...result, questions: verified };
  }
...
async function selectQuestions(archive, usedQuestions, targetDate, options = {}) {
  ...
  if (availableQuestions.length < ARCHIVE_SELECTION_ROUND_COUNT * 3) {
    throw new Error(`Not enough available questions. Need ${ARCHIVE_SELECTION_ROUND_COUNT * 3}, have ${availableQuestions.length}`);
  }
  ...
  for (let round = 0; round < numRounds; round++) {
    ...
    if (DIRECT_STRATEGY_ROUNDS.has(roundNumber)) {
...
async function generateGame(targetDate = null, forceOverwrite = false) {
  ...
  // Organize questions into rounds with direct per-round archive indexing
  const rounds = [];
  const questionsPerRound = 3;
  let archiveSliceIndex = 0;
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const roundQuestions = questions.slice(
      archiveSliceIndex * questionsPerRound,
      (archiveSliceIndex + 1) * questionsPerRound,
    );
    archiveSliceIndex++;
  ...
  attachAlternates(game, buildRoundAlternates({
    archive,
    rounds,
    usedQuestions: newUsedQuestions,
    bannedQuestions,
    poolLane: { selectListRoundQuestion, selectEntertainmentQuestions },
  }));
  ...
  saveUsedQuestionIds(Array.from(newUsedQuestions));
  console.log(`Updated used questions tracking (${newUsedQuestions.size} total)`);

  const ledger = loadQuestionLedger();
  ledger.usedIds = Array.from(newUsedQuestions);
